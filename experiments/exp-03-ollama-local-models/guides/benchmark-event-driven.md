# BENCHMARK OLLAMA — EVENT-DRIVEN ARCHITECTURE (Aider)
...
## PASSO 4 — Implementar a Task Manager API (Event-Driven)

### Conceito fundamental — Event-Driven Architecture

Cada mutação de estado (create, update, delete) publica um **Domain Event** imutável.
O `TaskService` não conhece os handlers — publica para o `EventBus` e pronto.
**SEM `ApplicationEventPublisher` do Spring** — implementar `EventBus` manual.
...
│   ├── TaskCreatedEvent.java      ← record imutável (todos os campos final)
...
├── service/
│   └── TaskService.java           ← publica exatamente 1 evento após cada operação de mutação
└── api/
    ├── TaskController.java
...
```powershell
cd $IMPL_DIR\src
# Verificar que InMemoryEventBus NÃO usa ApplicationEventPublisher
Select-String -Path (Get-ChildItem -Recurse -Filter InMemoryEventBus.java).FullName -Pattern "ApplicationEventPublisher"
# Deve retornar vazio

# Verificar que TaskService NÃO referencia handlers
Select-String -Path (Get-ChildItem -Recurse -Filter TaskService.java).FullName -Pattern "Handler"
# Deve retornar vazio
```

Nota de conformidade (0-10):
- `InMemoryEventBus` usa `ApplicationEventPublisher`? (sim = -3)
- `TaskService` conhece os handlers diretamente? (sim = -3)
- Events têm campos mutáveis (sem `final` / não são `record`)? (sim = -2)
- Cada mutação publica exatamente 1 evento? (não = -2)
...
- `EventBus` implementado sem Spring Events
- `TaskService` desacoplado dos handlers
