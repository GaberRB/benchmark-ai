# BENCHMARK JAVA QUARKUS — MAVEN MODO 1 (Agente Sequencial)
...
```powershell
python tools/snapshot.py --pre --language java-quarkus-maven
```

Confirme que o arquivo `tools/reports/snapshot_java-quarkus-maven_pre_*.json` foi criado.
...
Trabalhe dentro do diretório `experiments/exp-01-java-vs-kotlin/java-quarkus-maven-mode-1/`.

O `pom.xml` já está configurado — **não modifique**.
O `src/main/resources/application.properties` já está configurado — **não modifique**.

### Stack obrigatória

- Java 21, **Quarkus 3.8.3**, Maven
...
### service/TaskService.java (interface)
...
public class TaskServiceImpl implements TaskService {
    ...
    public Task update(String id, UpdateTaskRequest request) {
        ...
        if (request.getTitle() != null) {
            if (request.getTitle().isBlank()) throw new IllegalArgumentException("title is required");
...
public class TaskResource {
    ...
    @GET
    @Path("/{id}")
    public Task findById(@PathParam("id") String id) {
...
### exception/TaskNotFoundExceptionMapper.java
...
Adicione também um mapper para `ConstraintViolationException` (lançada pelo `@Valid`):
...
public class TaskResourceTest {
    ...
    void testGetTaskNotFound() {
        given()
            .when().get("/tasks/id-invalido-xyz")
            .then()
            .statusCode(404);
    }
    ...
    void testUpdateTaskNotFound() {
        given()
            .contentType(ContentType.JSON)
            .body("{\"title\":\"X\"}")
            .when().put("/tasks/id-invalido-xyz")
            .then()
            .statusCode(404);
    }
    ...
    void testDeleteTaskNotFound() {
        given()
            .when().delete("/tasks/id-invalido-xyz")
            .then()
            .statusCode(404);
    }
...
```powershell
cd experiments/exp-01-java-vs-kotlin/java-quarkus-maven-mode-1
.\mvnw.cmd compile
```
...
Cada falha corrigida é capturada como métrica automaticamente.
...
.\mvnw.cmd test
```
Esperado: `BUILD SUCCESS`, zero `Failures`, zero `Errors`
...
Quarkus sobe em modo produção com o jar compilado. Primeiro compile o pacote:
...
Alternativamente, use o modo dev (mais lento para iniciar, mas não exige compilar o pacote):

```powershell
# Não recomendado para E2E — use o jar compilado acima
```
...
Corrija o código, recompile (`.\mvnw.cmd compile`), e reinicie o app.
...
python tools/snapshot.py --post --language java-quarkus-maven --session-id <SESSION-ID>
python tools/collector.py --session-id <SESSION-ID> --language java-quarkus-maven
```

Confirme que `tools/reports/java-quarkus-maven_*.json` foi criado.
...
# Gerar relatório JaCoCo (já gerado durante .\mvnw.cmd test)
# O relatório está em: target/site/jacoco/index.html
# O CSV está em: target/site/jacoco/jacoco.csv

# Contar LOC
cd C:\Users\grios\OneDrive\Desktop\benchmark
cloc experiments/exp-01-java-vs-kotlin/java-quarkus-maven-mode-1/src/main/java --json | ConvertFrom-Json | Select-Object -ExpandProperty Java
cloc experiments/exp-01-java-vs-kotlin/java-quarkus-maven-mode-1/src/test/java --json | ConvertFrom-Json | Select-Object -ExpandProperty Java
```
...
Abra o arquivo `tools/reports/java-quarkus-maven_<timestamp>.json` e adicione/atualize os campos:
...
```
✅ BENCHMARK JAVA QUARKUS MAVEN MODO 1 — CONCLUÍDO

Session ID: <UUID>
Arquivo de métricas: tools/reports/java-quarkus-maven_<timestamp>.json
...
- `tools/reports/java-quarkus-maven_*.json` gerado e preenchido

---

## ⚠️ ERROS COMUNS COM QUARKUS
...
| Validação não retorna 400 | `@Valid` ausente no parâmetro | Adicione `@Valid` antes do parâmetro no método POST |
| Testes falham com "Ambiguous dependencies" | Dois beans do mesmo tipo | Use `@Default` ou remova implementação duplicada |
