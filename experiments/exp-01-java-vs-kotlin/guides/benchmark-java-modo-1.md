# BENCHMARK JAVA — MODO 1 (Agente Sequencial)
...
> - Você deve trabalhar **EXCLUSIVAMENTE** no diretório `java-mode-1/`
> - **NÃO leia, NÃO explore e NÃO referencie** nenhum outro diretório de implementação (`kotlin-*`, `java-gradle-*`, `java-quarkus-*`, etc.)
> - **NÃO crie** guias, arquivos de configuração ou diretórios fora de `java-mode-1/src/`
...
1. `.claude/spec/prd-java-task-manager-api.md` — PRD completo com mandatos, stack, estrutura e critérios
...
Trabalhe dentro do diretório `java-implementation/`.
...
```powershell
# Verificar cobertura JaCoCo — abrir no browser ou verificar via output
# O build falha se cobertura < 80% (configurado no pom.xml)
```
...
Corrija o código, rode `.\mvnw.cmd compile` e depois `.\mvnw.cmd test`, reaponte o app e repita o cenário que falhou.
...
# O relatório completo está em: target/site/jacoco/index.html

# Contar LOC
cd C:\Users\grios\OneDrive\Desktop\benchmark
cloc java-implementation/src/main/java --json | ConvertFrom-Json | Select-Object -ExpandProperty Java
cloc java-implementation/src/test/java --json | ConvertFrom-Json | Select-Object -ExpandProperty Java
```
