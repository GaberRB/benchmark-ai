# BENCHMARK KOTLIN QUARKUS — GRADLE MODO 1 (Agente Sequencial)
...
>
> **LINGUAGEM:** Kotlin · **FRAMEWORK:** Quarkus 3.8.3 · **BUILD:** Gradle 8.8
...
> - Você deve trabalhar **EXCLUSIVAMENTE** no diretório `kotlin-quarkus-gradle-mode-1/`
> - **NÃO leia, NÃO explore e NÃO referencie** nenhum outro diretório de implementação
> - **NÃO crie** guias, arquivos de configuração ou diretórios fora de `kotlin-quarkus-gradle-mode-1/src/`
> - Se você encontrar outros experimentos no filesystem, **ignore-os completamente**
> - Escreva **apenas arquivos `.kt`** — não crie arquivos `.java` nem `.groovy`
> - Todos os arquivos de build já existem — **não modifique** `build.gradle.kts`, `settings.gradle.kts`, `gradle.properties`
...
```powershell
python tools/snapshot.py --pre --language kotlin-quarkus-gradle
```
...
Trabalhe dentro de `experiments/exp-01-java-vs-kotlin/kotlin-quarkus-gradle-mode-1/`.

O `build.gradle.kts`, `settings.gradle.kts`, `gradle.properties` e `application.properties` já estão configurados — **não modifique**.

### Stack obrigatória

- Kotlin 1.9.22, **Quarkus 3.8.3**, Gradle 8.8
...
```
kotlin-quarkus-gradle-mode-1/
├── build.gradle.kts                  ← JÁ EXISTE — não modifique
├── settings.gradle.kts               ← JÁ EXISTE — não modifique
...
```powershell
cd experiments/exp-01-java-vs-kotlin/kotlin-quarkus-gradle-mode-1
...
.\gradlew.bat test
```
Esperado: `BUILD SUCCESSFUL`, zero failures, zero errors, cobertura ≥ 80%
...
python tools/snapshot.py --post --language kotlin-quarkus-gradle --session-id <SESSION-ID>
python tools/collector.py --session-id <SESSION-ID> --language kotlin-quarkus-gradle
```
...
cloc experiments/exp-01-java-vs-kotlin/kotlin-quarkus-gradle-mode-1/src/main/kotlin --json | ConvertFrom-Json | Select-Object -ExpandProperty Kotlin
cloc experiments/exp-01-java-vs-kotlin/kotlin-quarkus-gradle-mode-1/src/test/kotlin --json | ConvertFrom-Json | Select-Object -ExpandProperty Kotlin
```
...
```
✅ BENCHMARK KOTLIN QUARKUS GRADLE MODO 1 — CONCLUÍDO

Session ID: <UUID>
Arquivo: tools/reports/kotlin-quarkus-gradle_<timestamp>.json

[ ] .\gradlew.bat compileKotlin  — BUILD SUCCESSFUL
[ ] .\gradlew.bat test           — 0 failures, cobertura ≥ 80%
...
## ⚠️ ERROS COMUNS KOTLIN + QUARKUS GRADLE
...
| CDI falha em classe `final` | Kotlin fecha classes por padrão | O `build.gradle.kts` já configura `allOpen` — não modifique |
...
| `BuildFlowService` isolation | Kotlin 1.9.22 + Gradle 8.x bug | O `gradle.properties` já contém `kotlin.build.report.enable=false` |
| Toolchain not found | `jvmToolchain(21)` requer resolver | O `build.gradle.kts` já usa `kotlinOptions.jvmTarget` — não altere |
