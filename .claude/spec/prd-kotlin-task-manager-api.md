# PRD — Kotlin: Task Manager API

**Versão**: 2.0  
**Data**: 2026-06-04  
**Linguagem**: Kotlin 1.9+  
**Sessão**: `kotlin-implementation/`  
...
> **Windows**: substituir `./gradlew` por `gradlew.bat` se necessário.
...
Cada vez que `./gradlew compileKotlin` ou `./gradlew test` falhar, o agente deve:
...
- [ ] `./gradlew compileKotlin` — BUILD SUCCESSFUL, zero erros
- [ ] `./gradlew test` — BUILD SUCCESSFUL, zero failures, zero errors
- [ ] Cobertura Kover ≥ 80% de linhas
- [ ] `./gradlew bootRun` — aplicação sobe na porta 8080 sem erros
...
Implementar a Task Manager API em **Kotlin com Spring Boot 3.x**, seguindo exatamente a
especificação em `spec/task-definition.md`, de forma que os dados de tokens, erros e
velocidade coletados possam ser comparados de forma justa com a sessão Java.
...
| Cobertura        | Kover (Gradle plugin)                    |
| Storage          | In-memory (`ConcurrentHashMap`)          |
| IDs              | `UUID.randomUUID().toString()`           |
| Timestamps       | `Instant.now().toString()` → ISO 8601    |
...
### 6.2 Configuração Kover (build.gradle.kts)
...
kover {
    reports {
        verify {
            rule {
                minBound(80) // 80% de cobertura mínima de linhas
            }
        }
    }
}
...
# Relatório de cobertura disponível em: build/reports/kover/html/index.html
...
**O agente não deve declarar os testes unitários concluídos enquanto `./gradlew test` reportar
qualquer `FAILED` ou `X tests failed`.**

---

## 8. Testes E2E (Obrigatórios — app deve estar rodando)

Após `./gradlew test` passar com 100% de testes verdes, o agente deve:
...
2. Corrigir o código
3. Recompilar (`./gradlew compileKotlin`)
4. Reexecutar os testes unitários (`./gradlew test`)
5. Resubir o app e repetir o cenário E2E que falhou

**Passo 4** — Encerrar o app:

```bash
# Matar o processo Gradle/Spring Boot
pkill -f "bootRun" 2>/dev/null || kill $(lsof -t -i:8080) 2>/dev/null
```
...
A sessão Kotlin só está concluída quando **todos** os itens abaixo forem verificados:

| # | Critério | Comando de validação | Status |
|---|----------|----------------------|--------|
| 1 | Compilação sem erros | `./gradlew compileKotlin` → BUILD SUCCESSFUL | [ ] |
| 2 | Todos os testes unitários passam | `./gradlew test` → 0 failures, 0 errors | [ ] |
| 3 | Cobertura ≥ 80% | Kover report → Line coverage ≥ 80% | [ ] |
| 4 | App sobe sem erros | `./gradlew bootRun` → porta 8080 ok | [ ] |
...
Abrir Claude Code **dentro de `kotlin-implementation/`**:
...
```powershell
cd C:\Users\grios\OneDrive\Desktop\benchmark
python metrics/snapshot.py --post --language kotlin --session-id <UUID>
...
# Anotar Line coverage % e Branch coverage % de build/reports/kover/html/index.html
cloc src/main/kotlin --json
cloc src/test/kotlin --json
```
...
> Esta seção descreve como executar a sessão Kotlin no **Modo 2**, onde um agente orquestrador
...
|------|-----------|-----------------|
| T0 | Criar `build.gradle.kts` e `settings.gradle.kts` com dependências e plugins (Spring Boot, JUnit 5, Kover) | `build.gradle.kts`, `settings.gradle.kts` |
| T1 | Criar modelo `Task.kt` (data class) e DTOs (`CreateTaskRequest.kt`, `UpdateTaskRequest.kt`) | `model/`, `dto/` |
...
|------|-----------|-----------------|------------|
| T2 | Implementar `TaskService.kt` com storage in-memory | `service/TaskService.kt` | T1 |
| T3 | Implementar `TaskController.kt` com todos os 5 endpoints | `controller/TaskController.kt` | T1 |
| T4 | Implementar `TaskNotFoundException.kt` e `GlobalExceptionHandler.kt` | `exception/` | T1 |
...
| Task | Descrição | Arquivo(s) alvo | Depende de |
|------|-----------|-----------------|------------|
| T5 | Escrever testes unitários para `TaskService` (todos os cenários da Seção 7) | `service/TaskServiceTest.kt` | T2 |
| T6 | Escrever testes unitários para `TaskController` (todos os cenários da Seção 7) | `controller/TaskControllerTest.kt` | T3, T4 |

> T5 e T6 rodam em paralelo. Cada subagente deve rodar `./gradlew test` ao final e garantir
...
| Task | Descrição |
|------|-----------|
| T7 | Orquestrador integra o código, resolve conflitos, roda `./gradlew test` completo e executa os 12 cenários E2E da Seção 8 |
...
2. Ao finalizar: rodar `./gradlew compileKotlin` e, se aplicável, `./gradlew test`
...
```
Você é o orquestrador de uma sessão de benchmark Kotlin.
Sua missão:
1. Ler o PRD em .claude/spec/prd-kotlin-task-manager-api.md
...
python metrics/snapshot.py --post --language kotlin-mode2 --session-id <UUID-orquestrador>

# Coletar métricas do orquestrador
python metrics/collector.py --session-id <UUID-orquestrador> --language kotlin-mode2-orchestrator

# Coletar métricas de cada subagente
python metrics/collector.py --session-id <UUID-T2> --language kotlin-mode2-task-T2
python metrics/collector.py --session-id <UUID-T3> --language kotlin-mode2-task-T3
...
- Coroutines / fluxos reativos (usar implementação síncrona padrão)
