# BENCHMARK ARQUITETURA — LAYERED MVC (Agente Sequencial)
...
```powershell
python metrics/snapshot.py --pre --language arch-mvc
```

Confirme que `metrics/reports/snapshot_arch-mvc_pre_*.json` foi criado.
...
1. `.claude/spec/prd-arch-mvc.md` — PRD com estrutura de pacotes e regras de dependência
...
Trabalhe dentro do diretório `arch-benchmark/mvc/`.
...
```
arch-benchmark/mvc/src/main/java/com/benchmark/taskmanager/
...
python metrics/snapshot.py --post --language arch-mvc --session-id <SESSION-ID>
python metrics/collector.py --session-id <SESSION-ID> --language arch-mvc
```

Confirme que `metrics/reports/arch-mvc_*.json` foi criado.
...
cd C:\Users\grios\OneDrive\Desktop\benchmark
cloc arch-benchmark/mvc/src/main/java --json | ConvertFrom-Json | Select-Object -ExpandProperty Java
cloc arch-benchmark/mvc/src/test/java --json | ConvertFrom-Json | Select-Object -ExpandProperty Java
```
...
- Pacotes `controller/`, `service/`, `repository/`, `model/`, `dto/`, `exception/` presentes? (faltando algum = -1 cada)
...
Abra `metrics/reports/arch-mvc_<timestamp>.json` e adicione:
...
```
✅ BENCHMARK MVC — CONCLUÍDO

Session ID: <UUID>
Arquivo de métricas: metrics/reports/arch-mvc_<timestamp>.json
...
- `metrics/reports/arch-mvc_*.json` gerado e preenchido
- Estrutura: controller/, service/, repository/, model/, dto/, exception/
